...
// const markdownContent=ref<any>(`<img src=https://pandao.github.io/editor.md/images/logos/editormd-logo-180x180.png alt=Description style=width: auto; max-width: none;>`)
// const markdownContent=ref<any>(
// `
// <div style="overflow-x: auto;">
//   <img src="https://pandao.github.io/editor.md/images/logos/editormd-logo-180x180.png" alt="Description" style="width: auto; max-width: none;">
// </div>
// `)
...
const handleScroll = () => {
  //这样可以实现用户向上移了一下
  if (scrollContainer.value.clientHeight + scrollContainer.value.scrollTop < scrollContainer.value.scrollHeight - 20) {
    //用户向上滚动
    // console.log('用户向上滚动了')
...
//查询聊天记录
const queryChat = async (val: boolean) => {
  chatLoading.value = true;
  let params = {
...
//点击查询一段对话
const queryDetail = async (val: any, index: any) => {
  activeIndexFun(index)
  closeConnection()
  senfFlagLoading.value = false;
            ...
            <div v-for="(item, index) in listChat" :key="item">
              <div class="main_left_i" v-show="item.role == 'system'">
                ...
                <div class="main_left_hua">
                  ...
                  <!-- {{item.content}} -->
                  <!-- v-if="item.hasOwnProperty('think') && item.think.length > 0 && item.isSpread" -->
                  <!-- {{item.hasOwnProperty('think')}} -->
                  <div class="think_cla" v-if="item.think != '' && item.isSpread">
                    <MarkdownRenderer :markdown="item.think" class="think_cla11" />
                  </div>
                  <MarkdownRenderer :markdown="item.content" class="zw_cla" />
                  <div class="button_group" v-if="index == listChat.length - 1 && !isOut && !senfFlagLoading && !item.isHello
                  ">
                    <div class="g_copy">
                      <div>
